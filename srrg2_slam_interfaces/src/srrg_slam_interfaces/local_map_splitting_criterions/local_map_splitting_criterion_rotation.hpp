...
namespace srrg2_slam_interfaces {
  ...
  public:
    ...
    using VectorType                         = Vector_<float, AngularDimension>;
    using MatrixType                         = MatrixN_<float, Dimension>;
  ...
  protected:
    ...
    inline float _rotation2EulerDeltaChecked(const Matrix2f& rotation_) const {
      return std::fabs(atan2(rotation_(1, 0), rotation_(0, 0)));
    }
    inline float _rotation2EulerDeltaChecked(const Matrix3f& rotation_) const {
      Vector3f delta = rotation_.eulerAngles(0, 1, 2);
