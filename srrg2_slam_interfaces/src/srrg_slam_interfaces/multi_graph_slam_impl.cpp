...
namespace srrg2_slam_interfaces {
  ...
  void MultiGraphSLAM_<LoopClosureType_>::makeNewMap(float info_scale_) {
    ...
    const EstimateType prev_robot_pose     = robotPose();
  ...
  void MultiGraphSLAM_<LoopClosureType_>::loopValidate(
    GraphItemPtrSet_<LoopClosurePtrType>& reloc_closures_) {
  ...
  template <typename LoopClosureType_>
  bool MultiGraphSLAM_<LoopClosureType_>::putMessage(BaseSensorMessagePtr measurement_) {
  ...
  void MultiGraphSLAM_<LoopClosureType_>::relocalize(
    const GraphItemPtrSet_<LoopClosurePtrType>& reloc_closures_) {
    ...
    if (param_relocalizer->relocalizeMap()) {
      _current_local_map       = param_relocalizer->relocalizeMap();
      _robot_pose_in_local_map = param_relocalizer->poseInLocalMap();
  ...
  void MultiGraphSLAM_<LoopClosureType_>::draw(ViewerCanvasPtr canvas) const {
    ...
    // ds draw robot pose
    _current_local_map->setDrawingReferenceFrame(canvas, robotPose());
    canvas->putSphere(_current_local_map->size_local_map_shere);
