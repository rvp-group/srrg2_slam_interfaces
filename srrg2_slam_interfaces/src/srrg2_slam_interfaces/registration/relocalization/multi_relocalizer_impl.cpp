...
namespace srrg2_slam_interfaces {
  ...
  void MultiRelocalizer_<SlamAlgorithmType_>::compute() {
    ...
    ThisType::_robot_in_local_map  = EstimateType::Identity();
    ThisType::_relocalization_map  = nullptr;
    ...
    if (!aligner) {
      ...
      for (auto c : *(ThisType::_closure_candidates)) {
        ...
        if (best_closure) {
          if (c->chi_inliers > best_closure->chi_inliers) {
            DEBUG(relocalizer_debug)
              << ", " << FG_BRED("HIGH_CHI_INLIERS DROP: ") << c->chi_inliers << ")\n";
            continue;
          }
          if (c->num_correspondences < best_closure->num_correspondences) {
            DEBUG(relocalizer_debug) << ", " << FG_BRED("LOW_MIN_CORRESPONDENCE DROP: ")
                                     << c->num_correspondences << ")\n";
            continue;
          }
        }
        best_closure = c;
      ...
      if (best_closure) {
        ...
        ThisType::_relocalization_map  = best_closure->target();
        ThisType::_robot_in_local_map  = best_closure->pose_in_target;
      }

    } else {
      DEBUG(relocalizer_debug) << "MultiRelocalizer_::compute|search: [\n";

      aligner->setFixed(&tracker->measurementContainer());
      float best_chi_average = std::numeric_limits<float>::max();
      ...
      for (auto c : *(ThisType::_closure_candidates)) {
        ...
        aligner->setMovingInFixed(c->pose_in_target.inverse());
        aligner->compute();
        ...
        if (num_inliers < param_relocalize_min_inliers.value()) {
          DEBUG(relocalizer_debug) << ", " << FG_BRED("NUM_INLIERS DROP: ") << num_inliers << ")\n";
          continue;
        }
        ...
        if (curr_average < best_chi_average) {
          ThisType::_relocalization_map  = target_local_map;
          ThisType::_robot_in_local_map  = aligner->movingInFixed().inverse();
    ...
    if (ThisType::_relocalization_map) {
      std::cerr << FG_GREEN(" TARGET: ") << ThisType::_relocalization_map->graphId() << std::endl;
